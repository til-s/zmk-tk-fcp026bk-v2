...
├── D7(P1.12), D8(P1.13), D9(P1.14), D10(P1.15) → Col 4〜7  ※D6は未使用
...
**D4・D5はI2C専用のためFFC接続不可。D6は未使用。**  
D7〜D10の4本をCol 4〜7として使用。（D7はP1.12、D8はP1.13、D9はP1.14、D10はP1.15）

※当初はXIAOをRow・GPBをColとする配線で開始したが、GPA側キーの連続入力問題が解決できなかったため途中で上記の配線に変更した（詳細は第7章）。
...
- FFC/FPC変換基板 24ピン **1.0mmピッチ**（上接触 or 下接触は現物確認）
- ブレッドボード
- ジャンパーワイヤー
...
※当初のマトリクス解析時はFFC01〜08をRow・FFC17〜24をColとして接続していたが、GPA連続入力問題の解決のため配線を変更した。スキャンプログラムはFFC番号とピン名を表示するだけのため、配線変更後も同じプログラムを再スキャンに使用できる。
...
> 確定版（v6）。配線変更後・CapsLock LED・バッテリー報告・割り込みモード対応。

```dts
/*
 * TK-FCP026BK Shield Overlay（確定版）
 * XIAO nRF52840 + MCP23017
 *
 * Col 0-7 : XIAO GPIO D0-D3, D7-D10  ※D4=I2C(SDA), D5=I2C(SCL), D6は未使用
 ...
 *
 * poll-period-ms省略 → 割り込みモード有効（ColがXIAO直結のためnRF52840が直接検知）
 */
...
/ {
    ...
    indicator_leds: indicator_leds {
        ...
        caps_lock_led: caps_lock_led {
            leds = <&led2>;                         /* 青LED P0.06 */
            indicator = <HID_INDICATOR_CAPS_LOCK>;
            active-brightness = <100>;              /* CapsLock ON: 100% */
            inactive-brightness = <0>;              /* CapsLock OFF: 消灯 */
            disconnected-brightness = <0>;
        };
        disconnected_led: disconnected_led {
            leds = <&led0>;                         /* 赤LED P0.26 */
            indicator = <HID_INDICATOR_NUM_LOCK>;   /* ダミー: 未接続時表示用 */
            active-brightness = <0>;
            inactive-brightness = <0>;
            disconnected-brightness = <30>;         /* 未接続時: 赤30%点灯 */
        };
    };
...
> 16行×8列=128エントリ（Row行優先）。Col=XIAO(D0-D3,D7-D10)、Row=MCP23017(GPA0-7,GPB0-7)。

```dts
/*
 * TK-FCP026BK Keymap（確定版）
 * 86キー 日本語配列
 * bindingsの並び順: Row行優先 16行×8列=128エントリ
 * Col: 0(D0) 1(D1) 2(D2) 3(D3) 4(D7) 5(D8) 6(D9) 7(D10)
 */
...
/* Row13 GPB5 */ &kp KP_NUM        &trans            &trans            &kp LALT          &trans            &kp RALT          &trans            &kp PSCRN
...
/* Row15 GPB7 */ &trans            &kp LGUI          &trans            &trans            &trans            &trans            &trans            &trans
        ...
        fn_layer {
            bindings = <
/* Row0  GPA0 */ &trans            &trans            &trans            &trans            &trans            &bt BT_SEL 0      &trans            &bt BT_SEL 0
/* Row1  GPA1 */ &trans            &trans            &trans            &trans            &trans            &bt BT_SEL 1      &trans            &bt BT_SEL 1
/* Row2  GPA2 */ &trans            &trans            &trans            &trans            &trans            &bt BT_SEL 2      &trans            &bt BT_SEL 2
/* Row3  GPA3 */ &trans            &trans            &trans            &trans            &trans            &trans            &trans            &trans
...
/* Row9  GPB1 */ &trans            &trans            &trans            &trans            &trans            &kp PG_DN         &bt BT_CLR        &trans
...
config GPIO
    default y
config MCP230XX   # ← MCP23017はこのシンボル名（MCP230XXが正しい）
    default y
```
...
### 8-1. CapsLock LED（XIAO内蔵LED）

`zmk,indicator-leds` を使用してXIAO内蔵LEDでCapsLockを表示。

| 状態 | LED |
|---|---|
| BLE未接続 | 赤LED 30%点灯 |
| CapsLock OFF | 消灯 |
| CapsLock ON | 青LED 100%点灯 |
...
indicator_leds: indicator_leds {
    ...
    disconnected_led: disconnected_led {
        leds = <&led0>;  /* 赤LED P0.26 */
        indicator = <HID_INDICATOR_NUM_LOCK>;  /* ダミー */
        disconnected-brightness = <30>;
    };
};
```

Kconfig.defconfigへの追加：
```kconfig
config ZMK_HID_INDICATORS
    default y
config ZMK_BATTERY_REPORTING
...
col2row版overlayは `tk_fcp026bk.overlay.col2row` として記録済み。構成変更は未実施。
